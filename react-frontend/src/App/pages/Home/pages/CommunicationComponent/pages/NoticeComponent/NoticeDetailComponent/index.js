...
class InformationComponent extends Component {
    ...
    componentDidMount () {
        ...
        axios.get('/notice/'+locationArray[locationArray.length-1])
      .then(res => {
        const data = res.data;
        this.setState({data})
      }) 
      var prev =Number(locationArray[locationArray.length-1])-1 
        ...
        axios.get('/notice/'+next)
        .then(res => {
            const nextData = res.data;
            this.setState({nextData})
        })
        axios.get('/notice/'+prev)
        .then(res => {
                    ...
                    <table>
                        ...
                        <tr>
                            <td colspan="6" className={cx(`${moduleName}-next-prev`)}>
                              이전 글: {this.state.prevData.length===0?"이전 글이 없습니다.":this.state.prevData.map((post)=><a href={`/communication/notice/${post.no}`}>{post.title}</a>)}
                            </td>
                        </tr>
                        <tr>
                            <td colspan="6" className={cx(`${moduleName}-next-prev`)}>
                            다음 글: {this.state.nextData.length===0?"다음 글이 없습니다.":this.state.nextData.map((post)=><a href={`/communication/notice/${post.no}`}>{post.title}</a>)}
                            </td>
                        </tr>
