...
const moduleName = "SpaceRegisterComponent";

class SpaceRegisterComponent extends Component {
  constructor() {
    super();
    this.state = {
      data: [],
  ...
  buttonClick (event) {
    if (this.state.name !== ""&&this.state.phone!==""&&this.state.email!==""&&this.state.address!=="") {
      ...
      axios.post("/space_register", {user}).then(
        res => {
          console.log(res)
        }
      )
  ...
  componentDidMount () {
    ...
    axios.get('/space/'+locationArray[locationArray.length-1])
    .then(res => {
      const data = res.data
      this.setState({data})
    })
...
export default SpaceRegisterComponent